(function (root, factory) {
  ...
  function calculateValueFactors(input = {}) {
    ...
    if (isFirstDegreeEarthquakeZone(fields.earthquakeZone)) add("negative", "location-earthquake-first-degree", "1. Derece Deprem bölgesinde yer alıyor olması", "earthquakeZone", "Deprem Derecesi");
    const currentValueUsd = parseNumber(fields.currentValueUsd);
    if (Number.isFinite(currentValueUsd) && currentValueUsd > 1000000) {
      add("negative", "valuation-limited-buyer-pool", "Sınırlı bir alıcı kitlesine hitap ediyor olması", "currentValueUsd", "Mevcut Durum Değeri - USD");
    }
  ...
  function isFirstDegreeEarthquakeZone(value) {
    const text = fold(value).replace(/\s+/g, " ").trim();
    return /^(1)(?:\.|\s|$)/.test(text) || text.includes("1. DERECE") || text.includes("1 DERECE");
  }
